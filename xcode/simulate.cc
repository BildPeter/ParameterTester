...
namespace netevo {
   ...
   void SimulateMap::simulate (System &sys, double tMax, State &inital, SimObserver &obs, ChangeLog &logger) {
   ...
   void SimulateOdeFixed::simulate (System &sys, double tMax, State &inital, SimObserver &obs, ChangeLog &logger) {
      ...
      typedef odeint::runge_kutta4<State> rk4_stepper_type;
      typedef odeint::adams_bashforth_moulton<5,State> adams_bash_moul_stepper_type;
      
      // Solve the system
      switch (mStepper) {
         case RK_4:
            odeint::integrate_const(rk4_stepper_type(),
                                    Simulator(&sys), inital, 0.0, tMax, mStepSize, ObserverPassThrough(sys, obs, logger));
            break;
         case ADAM_BASH_MOUL:
            odeint::integrate_const(adams_bash_moul_stepper_type(), 
                                    Simulator(&sys), inital, 0.0, tMax, mStepSize, ObserverPassThrough(sys, obs, logger));
            break;
         default:
            // Do nothing
   ...
   void SimulateOdeConst::simulate (System &sys, double tMax, State &inital, SimObserver &obs, ChangeLog &logger) {
      ...
      switch (mStepper) {
         case RK_CASH_KARP_54:
            odeint::integrate_const(make_controlled( mEpsAbs , mEpsRel , rkck54_error_stepper_type() ), 
                                    Simulator(&sys), inital, 0.0, tMax, mOutputStep, ObserverPassThrough(sys, obs, logger));
            break;
         case RK_DOPRI_5:
            odeint::integrate_const(make_controlled( mEpsAbs , mEpsRel , dopri5_error_stepper_type() ), 
                                    Simulator(&sys), inital, 0.0, tMax, mOutputStep, ObserverPassThrough(sys, obs, logger));
            break;
         case RK_DOPRI_5_DENSE:
            odeint::integrate_const(make_dense_output( mEpsAbs , mEpsRel , dopri5_error_stepper_type() ),
                                    Simulator(&sys), inital, 0.0, tMax, mOutputStep, ObserverPassThrough(sys, obs, logger));
            break;
         default:
            // Do nothing
   ...
   void SimulateOdeAdaptive::simulate (System &sys, double tMax, State &inital, SimObserver &obs, ChangeLog &logger) {
      ...
      switch (mStepper) {
         case RK_CASH_KARP_54:
            odeint::integrate_adaptive(make_controlled( mEpsAbs , mEpsRel , rkck54_error_stepper_type() ), 
                                       Simulator(&sys), inital, 0.0, tMax, mInitialStep, ObserverPassThrough(sys, obs, logger));
            break;
         case RK_DOPRI_5:
            odeint::integrate_adaptive(make_controlled( mEpsAbs , mEpsRel , dopri5_error_stepper_type() ),
                                       Simulator(&sys), inital, 0.0, tMax, mInitialStep, ObserverPassThrough(sys, obs, logger));
            break;
         case RK_DOPRI_5_DENSE:
            odeint::integrate_adaptive(make_dense_output( mEpsAbs , mEpsRel , dopri5_error_stepper_type() ),
                                       Simulator(&sys), inital, 0.0, tMax, mInitialStep, ObserverPassThrough(sys, obs, logger));
            break;
         default:
            // Do nothing
